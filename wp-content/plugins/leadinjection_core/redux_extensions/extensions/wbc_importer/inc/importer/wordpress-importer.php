...
class WP_Import extends WP_Importer {
	...
	// PHP7 Fix for (function WP_Import())
...
<?php
	...
	function process_menu_item( $item ) {
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			// Immport custom meta data for Mega Main Menu
			foreach( $this->megamainmenu_custommeta_keys as $meta_key ) {
				if( !empty( ${$meta_key} ) ) {
					$meta_value = ${$meta_key};

					if ( is_serialized( $meta_value ) ) {
						leadinjection_debug_console( $meta_value );
	...
	// List of custom meta keys for Mega Main Menu
	var $megamainmenu_custommeta_keys = array(
		'mmm_item_descr',
		'mmm_item_style',
		'mmm_item_visibility',
		'mmm_item_icon',
		'mmm_disable_text',
		'mmm_disable_link',
		'mmm_pull_to_other_side',
		'mmm_submenu_type',
		'mmm_submenu_post_type',
		'mmm_submenu_drops_side',
		'mmm_submenu_columns',
		'mmm_submenu_enable_full_width',
		'mmm_submenu_bg_image',
	);
	...
	function fetch_remote_file( $url, $post ) {
		...
		if ( isset( $headers['content-type'] ) && "image/svg+xml" != $headers['content-type']  ) {
...
//		Old Statement / Changed for SVG Import (Leadinjection)
//		if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
//			@unlink( $upload['file'] );
//			return new WP_Error( 'import_file_error', __('Remote file is incorrect size', 'wordpress-importer') );
//		}
